...
```{r chunk1}
rm(list=ls())
total_data <- read.csv("/Users/TomTheIntern/Desktop/Final Lab/ClaimFreq.csv", header=T, stringsAsFactors=T)
...
```{r Ex1}
count_yes <- sum(total_data$Accident == 'Yes')
count_no <- sum(total_data$Accident == 'No')
...
The proportion of drivers having an accident in the current dataset is 15.79%.
...
```{r chunk2}
lm1 <- glm(Accident ~ Gender + HP + Credit + Years + 
             Marital + Vehicle + Miles + Family, 
...
Trucks have the highest chance of being in an accident. 

Good credit seems to be an indicator of being a better driver. 
...
gam1 <- gam(Accident ~ Gender + s(HP) + s(Credit) + s(Years) + 
              Marital + Vehicle + s(Miles) + s(Family), 
...
x_train_nn <- model.matrix( ~ ., data = train_data, na.rm = TRUE)[ , -12]
...
x_test_nn <- model.matrix( ~ ., data = test_data, na.rm = TRUE)[ , -12]

# standardization
x_test_nn <- scale(x_test_nn, center = x_mean, scale = x_sd)
x_test_nn <- x_test_nn[ , -1]

x_test_nn <- cbind.data.frame(test_data$Accident, x_test_nn)
colnames(x_test_nn)[1] <- 'Accident'
```
...
nn1_pred <- predict(nn1, newdata = x_test_nn)
...
nn1_acc <- confusionMatrix(factor(ifelse(nn1_pred > 0.5, 'Yes', 'No')), x_test_nn$Accident, positive = 'Yes')
```

**Exercise 6** Which model has the highest sensitivity and which one has the highest specificity?

lm Sensitivity 0.18613
lm Specificity 0.95779

gam Sensitivity 0.19161
gam Specificity 0.95617

nn Sensitivity 0.2956
nn Specificity 0.9357

Highest Sensitivity: NeuralNet
Highest Specificity: Log
...
```{r Ex8}
lift_chart <- lift(test_data$Accident ~ lm1_pred + gam1_pred + nn1_pred,
                   class = 'Yes', cuts = 200)
...
We would want to take in the % of drivers from 50% onward, based on the neural network model. So we would want to take anyone from 50% to 100%.
...
```{r chunk5}
rm(list=ls())
total_data <- read.csv("/Users/TomTheIntern/Desktop/Final Lab/ClaimSeverity.csv", header=T, stringsAsFactors=T)
total_obs <- dim(total_data)[1]
...
Area E would have the highest claim amount, followed by Area D. Area A, which is our base factor, would be third. The safest Area would be Area B
...
The gr1 model has the smallest MAE (793.2181). 
...
The sum of the test_data is \$10,447,480 while the lm model predicted just $6,539,062. This means that the lm model would have forecasted much less risk, meaning our company would have lost quite a lot of money. 

Meanwhile, the gamma model would have predicted $10,352,315 and so the risk would have been a much more accurate assessment. 
